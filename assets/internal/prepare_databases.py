...
rename_mapping_cant_hyd = {"hmm_file": "cant_hyd.hmm"}
...
rename_mapping_kofam = {"hmm_url": "kofam_profiles.hmm", "ko_list": "kofam_ko_list.tsv"}
...
rename_mapping_sulfur = {"hmm_file": "sulfur.hmm"}

rename_mapping_methyl = {"hmm_file": "methyl.faa"}
        ...
        for file in files:
            if file["type"] == "file" and file["name"] == "CANT-HYD.hmm":
                ...
                if file_response.status_code == 200:
                    ...
                    print(
                        f"Failed to download file: {file_name}. Status code: {file_response.status_code}"
                    )
    else:
        print(
            f"Failed to fetch file list from GitHub. Status code: {response.status_code}"
        )
        ...
        if database in database_urls:
            ...
            if database == "fegenie":
                ...
                for url_key, url_value in urls.items():
                    ...
                    download_file_name = path.basename(
                        download_url
                    )  # Extract the file name from the URL
                    download_file_path = path.join(
                        database_output_dir, download_file_name
                    )
        ...
        if path.exists(original_path):
            if original_file.endswith(".gz"):
                ...
                original_path = original_path[
                    :-3
                ]  # Remove .gz extension after decompression
            move(original_path, new_path)
            print(
                f"File {original_file} decompressed (if needed), renamed and moved to {new_path}"
            )
        else:
            print(f"File {original_file} not found. Skipping.")
...
def process_cant_hyd_database(output_dir):
    ...
    concatenated_hmm_path = path.join(
        cant_hyd_dir, "HMMs", "concatenated HMMs", "CANT-HYD.hmm"
    )
    ...
    if path.exists(concatenated_hmm_path):
        ...
        print("Concatenated HMM file not found in the repository.")
...
def process_kofam_database(output_dir, num_threads=1):
    ...
    if path.exists(ko_list_path):
        ...
        print("Decompressed and renamed ko_list to kofam_ko_list.tsv")
    else:
        ...
        if line.startswith("HMMER3/f"):
            if entry_started:
                ...
                sanitized_entry = process_entry(
                    current_entry, entry_names, name_suffix_counter
                )
...
def main():
    ...
    parser.add_argument(
        ...
        "-db",
        nargs="+",
        help="List of databases to update",
        required=True,
    )
    parser.add_argument(
        "--threads",
    ...
    for database in args.databases:
        ...
        if database == "vogdb":
            process_vogdb_database(
                path.join(database_output_dir, "vog.hmm.tar.gz"),
                database_output_dir,
                args.threads,
            )
            concatenate_hmm_files(
                path.join(database_output_dir, "hmm"),
                path.join(database_output_dir, "vog_latest_hmms.hmm"),
            )
            ...
            create_mmseqs(
                path.join(database_output_dir, "mmseqs", "camper_blast.faa"),
                path.join(database_output_dir, "mmseqs", "camper.mmsdb"),
                args.threads,
            )
        elif database == "pfam":
            ...
            create_mmseqs(
                path.join(database_output_dir, "methyl.faa"),
                path.join(database_output_dir, "methyl"),
                args.threads,
            )
        elif database == "dbcan":
