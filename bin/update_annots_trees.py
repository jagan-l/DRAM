...
def update_tsv(tsv_path, output_tsv_path, placement_map):
    ...
    col_order_start = df.columns.tolist()[: df.columns.get_loc("gene_number") + 1] + [
        "tree_verified"
    ]
...
def main():
    if len(sys.argv) != 5:
        print(
            "Usage: python update_annots_trees.py <jplace_path> <tsv_path> <mapping_tsv> <output_tsv_path>"
        )
        sys.exit(1)
