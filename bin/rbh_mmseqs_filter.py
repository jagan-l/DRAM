...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Filter Reciprocal Best Hits (RBH) from forward and reverse MMseqs2 search results."
    )
    parser.add_argument(
        "--forward",
        type=str,
        required=True,
        help="File path to forward search results.",
    )
    parser.add_argument(
        "--reverse",
        type=str,
        required=True,
        help="File path to reverse search results.",
    )
    parser.add_argument(
        "--output",
        type=str,
        required=True,
        help="Output file path for combined RBH results.",
    )
