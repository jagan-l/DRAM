...
def fetch_descriptions(chunk, db_name, db_file):
    ...
    if db_name == "dbcan":
        ...
        chunk[f"{db_name}_description"] = chunk[hits_ids_column].map(
            lambda x: descriptions_dict.get(x, ("", ""))[0]
        )
        chunk["dbcan_EC"] = chunk[hits_ids_column].map(
            lambda x: format_dbcan_EC(descriptions_dict.get(x, ("", ""))[1])
        )
    elif db_name == "kegg":
        # For other databases, just update with descriptions
        descriptions_dict = {row[0]: row[1] for row in results}
        chunk[f"{db_name}_description"] = chunk[hits_ids_column].map(
            lambda x: descriptions_dict.get(x, "")
        )
        chunk["kegg_orthology"] = chunk[f"{db_name}_description"].apply(
            extract_kegg_orthology
        )
        ...
        if "kegg_id" not in chunk.columns:
            chunk["kegg_id"] = (
                ""  # or any default logic you have for generating 'kegg_id'
            )
...
def format_dbcan_EC(ec_string):
    ...
    if ec_string:
        ...
        formatted_ec_numbers = "; ".join(
            [f"EC:{ec}" for ec in ec_numbers if ec]
        )  # Add "EC:" prefix and join with semicolon
...
def extract_kegg_EC(description):
    ...
    if ec_start != -1:
        ...
        ec_numbers = re.findall(
            r"\b\d+\.\d+\.\d+\.\d+\b", ec_text
        )  # Extract EC numbers using regex
        formatted_ec_numbers = "; ".join(
            [f"EC:{ec}" for ec in ec_numbers]
        )  # Add "EC:" prefix and join with semicolon
        return formatted_ec_numbers
    else:
        return None
...
def main():
    parser = argparse.ArgumentParser(
        description="Add descriptions from SQL database to hits file"
    )
    parser.add_argument("--hits_csv", type=str, help="Path to the hits CSV file")
    parser.add_argument(
        "--db_name",
        type=str,
        help="Name of the database table to fetch descriptions from",
    )
    parser.add_argument(
        "--output", type=str, help="Path to the output formatted CSV file"
    )
    ...
    with ThreadPoolExecutor() as executor:
        ...
        processed_chunks = executor.map(
            lambda chunk: fetch_descriptions(chunk, args.db_name, args.db_file),
            list(reader),
        )
