...
HMMSCAN_ALL_COLUMNS = ['query_id', 'target_ascession', 'target_length', 'query_name', 'query_ascession', 'query_length', 'full_evalue', 'full_bitScore', 'full_bias', 'domain_number', 'domain_count', 'domain_cevalue', 'domain_ievalue', 'domain_bitScore', 'domain_bias', 'model_start', 'model_end', 'alignment_start', 'alignment_end', 'env_start', 'env_end', 'accuracy', 'description']
...
OUTPUT_COLUMNS = ['query_id', 'start_position', 'stop_position', 'strandedness', '{db_name}_id', '{db_name}_ids', "{db_name}_full_bitScore", "{db_name}_domain_bitScore", 'description', '{db_name}_EC', "{db_name}_score_rank"]
...
def sig_scores_row_by_row(df, db_name) -> pd.DataFrame:
    ...
    if "threshold" in df.columns:
        ...
        raise ValueError(f"No threshold or A_rank columns found in provided info sheet to determine significant hits for {db_name} hmm DB.")
    return df    
...
@click.command()
@click.option("--hmm_domtbl", type=click.Path(exists=True), help="Path to the HMM search results domtblout file.")
@click.option("--hmm_info_path", type=click.Path(exists=True), help="Path to the hmm_info_path file.")
@click.option("--ec_from_info", is_flag=True, help="Extract EC numbers from definitions in info sheet. Can only be used if hmm_info_path is provided.")
...
def main(hmm_domtbl, hmm_info_path, ec_from_info, gene_locs, db_name, output):
    ...
    hits['perc_cov'] = (hits['model_end'] - hits['model_start'] + 1) / hits['query_length']
    ...
    if hmm_info_path is not None:
        ...
        try:
            ...
            try:
                hmm_info = pd.read_csv(hmm_info_path, sep="\t", index_col=0, on_bad_lines='skip')
    ...
    if hmm_sheet and not hmm_info.empty:
        ...
        if ec_from_info:
            if raise_on_ec:
                raise ValueError("No suitable column found for descriptions in hmm_info_path.")
            hmm_info[f"{db_name}_EC"] = hmm_info["description"].apply(extract_ec_numbers)
