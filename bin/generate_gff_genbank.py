...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Generate GFF and/or GBK files from raw annotations, with specified databases formatting."
    )
    ...
    parser.add_argument(
        "--input_fastas_paths",
        nargs="+",
        help="Alternating list of input_fasta names and paths to their .fna files.",
    )
    parser.add_argument(
        "--database_list",
        type=str,
        help="Comma-separated list of databases to include in the annotations. Use 'empty' for all.",
        default="empty",
    )
    parser.add_argument(
        "--annotations", required=True, help="Path to the raw annotations file"
    )
    args = parser.parse_args()
    args.database_list = (
        None
        if (args.database_list == "empty" or args.database_list == "all")
        else args.database_list.split(",")
    )
    return args
...
def generate_gff(input_fastas_annotations, database_list):
    ...
    for input_fasta, annotations in input_fastas_annotations.items():
        with open(f"GFF/{input_fasta}.gff", "w") as gff_file:
            ...
            taxonomy = escape_gff3_value(
                metadata.get("taxonomy", "NA").replace(";", ",")
            )
            ...
            for annotation in annotations:
                try:
                    ...
                    logging.warning(
                        f"Missing key in annotation: {e}. Skipping annotation: {annotation}"
                    )
    ...
    for input_fasta, file_path in input_fastas_and_paths.items():
        if os.path.exists(file_path):
            ...
            sequences[input_fasta] = [
                seq_record.seq for seq_record in SeqIO.parse(file_path, "fasta")
            ]
        else:
    ...
    qualifiers = {
        ...
        "product": next(
            (
                ...
                if k.endswith("_description") and annotation[k]
            ),
            "unknown product",
        ),
    }
...
def generate_gbk(input_fastas_annotations, database_list, input_fastas_and_paths):
    ...
    for input_fasta, annotations in input_fastas_annotations.items():
        ...
        input_fasta_seq_record = SeqRecord(
            ...
            id=input_fasta,
            name="",
            description=f"Annotations for {input_fasta}",
        )
        ...
        input_fasta_seq_record.annotations["organism"] = metadata.get(
            "taxonomy", "Not Available"
        )
        input_fasta_seq_record.annotations["comment"] = (
            f"Completeness: {metadata.get('Completeness', 'NA')}; Contamination: {metadata.get('Contamination', 'NA')}; Taxonomy: {metadata.get('taxonomy', 'Not Available')}"
        )
        ...
        for annotation in annotations:
            ...
            if query_id in sequence_index:
                ...
                print(
                    f"Warning: Query ID {query_id} not found in .fna file for input_fasta {input_fasta}."
                )
...
def main():
    ...
    if args.gbk:
        generate_gbk(
            input_fastas_annotations, args.database_list, input_fastas_and_paths
        )
